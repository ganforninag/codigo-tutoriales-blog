...
public class JwtUtil {
    ...
    static void addAuthentication(HttpServletResponse res, String username, Collection<? extends GrantedAuthority> authorities) {
        ...
        String roles = authorities.parallelStream().map(auth -> (GrantedAuthority) auth).map(a -> a.getAuthority()).collect(Collectors.joining(","));
    ...
    static Authentication getAuthentication(HttpServletRequest request) {
        ...
        if (token != null) {
            ...
            List authorities = Stream.of(roles.split(","))
